...
export class ResultsComponent implements OnInit {
        ...
        if (_.isEmpty(results.params) && !_.isEmpty(results.queryParams)) {
          ...
          if (params && params["type"]) {
            ...
            if (this.currentTypeDetails === "who") {
              ...
              this.currentTypeDetails === "where" ||
              this.currentTypeDetails === "group"
            ) {
  ...
  fillRows() {
    if (this.currentTypeDetails === "plain-search") {
      ...
      this.dataService
        .getTotalNoOfApps()
        .subscribe((data: any) => (this.totalSize = data));
  ...
  onColumnClicked(column: any) {
    if (column.id === this.activeSorterCol) {
      this.activeSorterDirection =
        this.activeSorterDirection === "ASC" ? "DESC" : "ASC";
    } else {
      this.activeSorterCol = column.id;
    }
    this.rowGroups = SortRowGroups(
      this.rowGroups,
      this.activeSorterCol,
      this.activeSorterDirection
    );
  }
  ...
  loadNextPage(event: PageEvent | null) {
    ...
    switch (this.fetchCallType) {
      case FetchType.PLAIN_SEARCH: {
        this.dataService
          .getRelationships(this.skip, this.limit)
          .subscribe((data: any) => this.buildRows(data));
        break;
      }
      case FetchType.WHO: {
        // tslint:disable-next-line:max-line-length
        this.dataService
          .getAppRelationships(this.paramId, this.skip, this.limit)
      ...
      case FetchType.WHERE: {
        // tslint:disable-next-line:max-line-length
        this.dataService
          .getHostRelationships(this.paramId, this.skip, this.limit)
          .subscribe((data: any) => this.buildRows(data, true));
        break;
      }
      case FetchType.GROUP: {
        // tslint:disable-next-line:max-line-length
        this.dataService
          .getGroupRelationships(this.paramId, this.skip, this.limit)
          .subscribe((data: any) => this.buildRows(data, true));
        break;
      }
      case FetchType.COMPOSITE_SEARCH: {
        // tslint:disable-next-line:max-line-length
        this.dataService
          .getBaseRelationships(this.currentQueryParams, this.skip, this.limit)
          .subscribe((data: any) => {
            this.buildRows(data);
          });
        break;
  ...
  buildRows(data: Array<any>, forceExpand = true) {
    ...
    // this.rowGroups2 = BuildRowGroups2(data, forceExpand);
    if (this.currentTypeDetails === "who") {
  ...
  buildPurposeDistribution() {
    ...
    this.appService.showLoader();
    this.dataService.getPurposeDistribution(params).subscribe(
      (data: any) => {
        this.appService.hideLoader();
        const rows: any[] = [];
  ...
  buildCategoryWiseDistribution() {
    ...
    if (this.currentTypeDetails === "where") {
      params["hosts"] = [this.paramId];
      this.appService.showLoader();
      this.dataService.getCategoryWiseDistribution(params).subscribe(
        (data: any) => {
          this.appService.hideLoader();
          const rows: any = [];
          if (data && data.length > 0) {
              ...
              switch (prop) {
                case "POPULAR_PURPOSE": {
                  ...
                  this.statistics.push({
                    key: "The most popular reason for sending data ",
                    value: purposeInfo.shortLabel
                  });
                  break;
                }
                case "POPULAR_TYPE": {
                  ...
                  this.statistics.push({
                    key: "The type of data being sent most frequently",
                ...
                case "POPULAR_HOST": {
                  ...
                  this.statistics.push({
                    key: "The most frequent destination for data",
                    value: hostData.host
                  });
                  break;
                }
                case "POPULAR_APP": {
                  ...
                  this.statistics.push({
                    key: "The most recurring app sending app",
                    value: appData.title ? appData.title : appData.app
                  });
                  break;
                }
              }
